'''
  ...
  # Detect the corner of chessboard (Ver.2)
...
def findChessboardCorners(img, annots, pattern):
    ...
    for factor_ in factor:
        img = cv2.resize(img, dsize=(int(1998*factor_), int(1125*factor_)), interpolation=cv2.INTER_AREA) 
        ...
        for func in [_findChessboardCornersAdapt, _findChessboardCorners]:
            ...
            if ret:
                ...
                temp_2dkey=rescaling(corners.tolist(),[1279,720],[img.shape[1],img.shape[0]])
